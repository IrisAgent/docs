...
1. Go to **AI Products** → **Chatbot** and click **Configure and deploy**. The default chatbot configuration opens automatically; use the **Configuration** selector to switch configurations or **New configuration** to create one.
    <img
...
3. Click **Deploy**, then **Reveal API Key**, and paste the snippet into the **body** section of your website.
...
4. You can see chatbot conversations under **Activity** → **Conversations**.
...
- **From the chatbot configuration editor** (recommended for most users): edit the common settings through the dashboard UI. No code required.
...
These options are organized into **Instructions**, **Behavior**, **Appearance**, **Handoff**, **Localization**, and **Advanced** tabs in the chatbot configuration editor.

| Setting | Description | Default |
| --- | --- | --- |
| **Configuration name** | Name shown in the chatbot header and used by your team to identify the configuration. | — |
...
| **Agent handoff** | What happens when a visitor needs a human: send the support team's contact details, open a support ticket, or start a live chat. Includes an optional handoff message. | Send contact details |
...
<Note>
Setting **Agent handoff** to **Open a ticket** requires a ticketing or CRM data
source to be connected first, so IrisAgent has somewhere to create the ticket.
If your account was set up from a website link only, connect a source such as
[Salesforce](/data-sources/Salesforce) before selecting this option. Connecting
Salesforce uses its own Salesforce sign-in and is separate from your original
website signup.
</Note>

<Note>
To let visitors stay in the IrisGPT widget while agents reply from Salesforce,
first configure an Enhanced Chat custom client deployment, then select **Start a
live chat** under **Agent handoff**. Follow the [Salesforce Live Chat Handoff
guide](/deploying-irisagent/Salesforce-Live-Chat) for the Salesforce and
IrisAgent setup steps.
</Note>

<Note>
For Zendesk Messaging webhook deployments, select **Start a live chat** in the
**Handoff** tab and optionally enter Zendesk ticket tags. When a procedure
escalates, IrisAgent adds the tags to the active messaging ticket and passes
Switchboard control to Zendesk Agent Workspace. See [Chatbot Automation on
Zendesk Messaging](/deploying-irisagent/Chatbot-Automation-on-Zendesk-Messaging#step-5-configure-human-handoff-and-zendesk-routing).
</Note>
