...
4. Select `new_request_page.hbs` from the **Templates** section and paste the following code snippet after the closing `</div>` of the main container:
...
6. Since the IrisAgent search bar replaces the built-in one, remove the entire `<div class="search-container">` element and everything inside it.

<Accordion title="Full example of new_request_page.hbs">
```html
