...
Employees can also ask questions through [Internal AI Search in Confluence](/deploying-irisagent/Internal-AI-Search-on-Confluence). The embedded search experience uses the Agent Assist answer pipeline with employee IrisAgent sign-in, source links, and follow-up questions. It requires the employee-search release and does not receive the current ticket's context.
